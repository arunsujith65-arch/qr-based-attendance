...
export default function StaffDashboard() {
  ...
  const handleCreateStudent = async (e: React.FormEvent) => {
    ...
    try {
      if (editingStudent) {
        ...
        const instanceId = profile.instanceId || 'legacy';
  ...
  }, [activeSession, profile, students]);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (diffInMinutes >= 15 && activeSession.status === 'active') {
        endSession();
      }
    }, 30000);

    return () => clearInterval(timer);
  }, [activeSession, profile, endSession]);
  ...
  const markStudentPresent = async (student: StudentAccount, customSession?: Session | SessionRecord) => {
    const sessionToUse = customSession || selectedHistorySession;
    if (!sessionToUse || !profile) return;
    
    try {
      // Check if already marked
      const q = query(
        collection(db, 'attendance'), 
        where('sessionId', '==', sessionToUse.id),
        where('studentId', '==', student.registerNo)
      );
      ...
      if (existingSnap.empty) {
        await addDoc(collection(db, 'attendance'), {
          ...
          status: 'present',
          timestamp: serverTimestamp(),
          manuallyMarkedBy: profile.staffId
        });
      } else {
        // If it was 'absent' before, update it to 'present'
        const existingDoc = existingSnap.docs[0];
        if (existingDoc.data().status === 'absent') {
  ...
  const unmarkStudent = async (student: StudentAccount, customSession?: Session | SessionRecord) => {
    const sessionToUse = customSession || selectedHistorySession;
    if (!sessionToUse) return;
    try {
      // Use active attendance local state if possible, or historyAttendance
      const targetList = customSession ? (customSession.status === 'active' ? attendance : historyAttendance) : historyAttendance;
      const record = targetList.find(a => a.studentId === student.registerNo);
      if (record) {
                ...
                <div className="flex-1 overflow-y-auto p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-bold text-gray-900 flex items-center gap-2">
                       Roll Call ({attendance.length} / {students.length})
                    </h3>
                  </div>
                  <div className="space-y-2">
                    {students.map((student) => {
                      const record = attendance.find(a => a.studentId === student.registerNo);
                      const isPresent = record?.status === 'present';

                      return (
                        <div key={student.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${isPresent ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-100 opacity-60'}`}>
                           <div className="w-8 h-8 rounded-full flex items-center justify-center bg-white border font-bold text-xs">
                             {student.name[0]}
                           </div>
                           <div className="flex-1">
                             <p className="font-bold text-sm text-gray-900">{student.name}</p>
                             <p className="text-[10px] text-gray-500">{student.registerNo}</p>
                           </div>
                           {isPresent ? (
                             <button
                               onClick={() => unmarkStudent(student, activeSession!)}
                               className="p-1 px-2 bg-red-100 text-red-600 rounded-lg text-[10px] font-bold"
                             >
                               Unmark
                             </button>
                           ) : (
                             <button
                               onClick={() => markStudentPresent(student, activeSession!)}
                               className="p-1 px-2 bg-green-600 text-white rounded-lg text-[10px] font-bold"
                             >
                               Mark Present
                             </button>
                           )}
                    ...
                    {students.length === 0 && (
                      <p className="text-center text-gray-400 text-sm py-12">No students registered yet.</p>
                    )}
                  </div>
                </div>
              </motion.div>
